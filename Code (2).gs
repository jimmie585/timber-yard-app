...
function addQuotation(body) {
  ...
  return { data: getAllData(), quoteId: quoteId, quoteNo: quoteNo, emailSent: emailSent };
}
...
function convertQuotation(body) {
  ...
  var saleResult = addSale({ customerName: customerName, phone: phone, email: email, items: items, paid: body.paid || 0 });
